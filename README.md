...
| 스크립트 | 용도 |
|---|---|
| `scripts/verify-standalone.sh` | CloudCompare 없이 빠른 검증 — DxfWriter 컴파일·실행 + interpret.py(L4) 테스트 |
| `scripts/integrate.sh <CC_SRC>` | plugin/을 CloudCompare 트리에 배치 + CMake 등록 (멱등) |
| `scripts/build-linux.sh [CC_SRC] [ref]` | Linux에서 CloudCompare+플러그인 빌드 (C++ 컴파일 검증) |
| `scripts/build-windows.ps1 -QtDir ...` | Windows 빌드 (실제 배포 타깃) |
| `.github/workflows/ci.yml` | CI: standalone(항상) + Linux/Windows 전체 빌드 |

> `verify-standalone.sh`와 CI standalone 잡은 이미 통과 확인됨. Linux/Windows 전체
> 빌드 잡은 실제 CloudCompare 헤더로 C++ API 정합성을 검증한다(첫 실행 시 조정 가능성).
